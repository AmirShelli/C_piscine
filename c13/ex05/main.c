...
void *btree_search_item(t_btree *root, char *data_ref, int (*cmpf)(char *, char *));
...
int main()
{
	...
	printf("\n(%s) => end\n",tree->item);
	t_btree *search = btree_search_item(tree, "D", &ft_strcmp);
	printf("%s)\n",search->item);
	ft_putstr(((struct s_btree *)(btree_search_item(tree, "D", &ft_strcmp)))->item);
}